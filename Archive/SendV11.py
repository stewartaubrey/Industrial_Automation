...
def update_file_list():
    try:
        ...
        # Get file modification times and sort by newest to oldest
        file_list_with_times = []
        for file in file_list:
            try:
                # Assuming the server sends the modification time along with the file name
                # in the format "filename|timestamp"
                name, timestamp = file.split('|')
                mod_time = datetime.datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S')
                file_list_with_times.append((name, mod_time))
            except ValueError:
                # If the format is incorrect, skip this file
                continue

        # Sort files by modification time (newest first)
        file_list_with_times.sort(key=lambda x: x[1], reverse=True)

        # Extract sorted file names
        sorted_file_list = [file[0] for file in file_list_with_times]
        
        file_combobox['values'] = sorted_file_list
    except Exception as e:
        print(f"Error: {e}")
...
combobox_title = tk.Label(root, text="Select File to Send to CNC or Delete from ESP32", font=("Helvetica", 14))
